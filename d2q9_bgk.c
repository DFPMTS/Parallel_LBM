...
int collision(int row, int start_col, int end_col, const t_param params,
              t_speed *cells, t_speed *tmp_cells, int *obstacles);
int streaming(int row, int start_col, int end_col, const t_param params,
              t_speed *cells, t_speed *tmp_cells);
int streaming_col(int col, int start_row, int end_row, const t_param params,
                  t_speed *cells, t_speed *tmp_cells);
void step(int start_row, int end_row, int start_col, int end_col,
          const t_param params, t_speed *cells, t_speed *tmp_cells,
          int *obstacles);
...
#pragma omp parallel for
  for (int i = 0; i < chunks; ++i) {
    ...
    collision(1, start, end, params, cells, tmp_cells, obstacles);
    // for (int i = 2; i < params.ny; ++i) {
    // collision(i, start, end, params, cells, tmp_cells, obstacles);
    // streaming(i - 1, start + 1, end - 1, params, cells, tmp_cells);
    // }
    step(2, params.ny, start, end, params, cells, tmp_cells, obstacles);
...
void step(int start_row, int end_row, int start_col, int end_col,
  ...
  ** are in the scratch-space grid */
  ...
  for (int jj = start_row; jj < end_row; ++jj) {
    ...
    jj--;
    for (int ii = start_col + 1; ii < end_col - 1; ii++) {
      /* determine indices of axis-direction neighbours
      ** respecting periodic boundary conditions (wrap around) */
      int y_n = ((jj + 1) >= params.ny) ? 0 : jj + 1;
      int x_e = ((ii + 1) >= params.nx) ? 0 : ii + 1;
...
/*
** Work with boundary conditions. The upper and lower boundaries use the
*rebound plane,
